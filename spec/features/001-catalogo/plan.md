...
4. **Formularios seguros — `inventario/forms.py`**: Crear `CategoriaForm`, `ProductoCrearForm` y `ProductoEditarForm`. Excluir deliberadamente `stock_actual` de los formularios de productos para cumplir con la regla de negocio. Permitir en `ProductoCrearForm` el campo `stock_inicial` opcional que delega en el servicio `registrar_entrada` asegurando la trazabilidad desde el alta inicial.
